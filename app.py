...
with tab_basis:
    ...
    st.caption('QQQ 1999년~현재 데이터로 본 나스닥 조정 패턴과 전략 강건성 검증')
    ...
    if qqq_hist is not None:
        mdd_series, _ = calc_mdd_series(qqq_hist)
        import plotly.graph_objects as go

        # ── 에피소드 추출 (공통): QQQ -5% 이하 진입 ~ ATH 회복 ──
        def extract_episodes_bucketed(mdd_s):
            """실제 저점 기준으로 에피소드 추출."""
            episodes = []
        ...
        all_eps = extract_episodes_bucketed(mdd_series)
        completed_eps = [e for e in all_eps if e['완료']]
        ongoing_eps   = [e for e in all_eps if not e['완료']]
        total_years   = (qqq_hist.index[-1] - qqq_hist.index[0]).days / 365

        # ── ① QQQ 조정 깊이 분포 ──
        st.subheader('① QQQ 조정 깊이 분포 (1999년 ~ 현재)')
        st.caption('에피소드 = QQQ가 ATH 대비 -5% 이하 진입 후 신고가 회복까지. "실제 저점"이 어느 구간에 속했는지 기준으로 분류.')
        ...
        st.caption('실제 TQQQ 2010~현재 데이터. 비중을 얕은 구간 집중(초반) → 균등 → 깊은 구간 집중(후반)으로 극단까지 바꿔봐도 성과 차이가 미미합니다.')

        # 그리드 서치 결과 (하드코딩 — optimize_single.py 실행 결과)
        robustness_data = [
            {'비중 방식': '얕은 집중 (극단)', 'CAGR': 40.9, 'Sharpe': 0.954, 'MDD': -77.7, '특징': '-5%에 65% 집중'},
            {'비중 방식': '얕은 집중',         'CAGR': 40.6, 'Sharpe': 0.958, 'MDD': -76.8, '특징': '-5%에 34% 집중'},
            {'비중 방식': '균등 배분',          'CAGR': 40.1, 'Sharpe': 0.967, 'MDD': -75.4, '특징': '각 레벨 10%씩'},
            {'비중 방식': '깊은 집중',          'CAGR': 39.8, 'Sharpe': 0.976, 'MDD': -74.7, '특징': '-50%에 18% 집중'},
            {'비중 방식': '깊은 집중 (극단)',   'CAGR': 39.5, 'Sharpe': 0.980, 'MDD': -73.9, '특징': '-50%에 26% 집중'},
        ]
        rob_df = pd.DataFrame(robustness_data)
        rob_df['CAGR'] = rob_df['CAGR'].apply(lambda x: f'{x:.1f}%')
        rob_df['Sharpe'] = rob_df['Sharpe'].apply(lambda x: f'{x:.3f}')
        rob_df['MDD'] = rob_df['MDD'].apply(lambda x: f'{x:.1f}%')
        st.dataframe(rob_df, use_container_width=True, hide_index=True)

        col_r1, col_r2, col_r3 = st.columns(3)
        col_r1.metric('CAGR 최대 차이', '1.4%p', help='극단 얕은 집중 vs 극단 깊은 집중')
...
> 초반/중반/후반 집중형의 차이는 투자자 심리 관리 용도에 가깝습니다.
''')
        ...
        if eps_sel:
            ...
            st.caption(f'총 {len(eps_sel)}개 | 회복 기준 = QQQ 신고가 달성')
        else:
...
MDD -30% 이하 에피소드는 {len([e for e in all_eps if e['저점'] <= -30])}회, 평균 발생 간격은 약 {round(total_years / max(len([e for e in all_eps if e['저점'] <= -30]), 1), 1)}년입니다.
