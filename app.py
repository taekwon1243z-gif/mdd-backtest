    ...
    with st.expander('② 전략 비교 결과', expanded=st.session_state.step == 2):
        ...
        for name in strategy_names:
            ...
            with st.expander(f'{name} | 최종 {final_krw/10000:.0f}만원 | 연평균 {cagr:.1f}%'):
                ...
                # ── insight ──
                if ep_rate is not None and ep_rate >= 60 and calmar >= 0.5:
                    insight = (f'✅ 폭락 에피소드 {ep_total}개 중 {ep_wins}개에서 홀딩을 이겼습니다 '
                               f'(승률 {ep_rate:.0f}%). Calmar {calmar:.2f} — 리스크 대비 수익 효율도 양호합니다.')
                elif ep_rate is not None and ep_rate >= 60:
                    insight = (f'📈 에피소드 승률 {ep_rate:.0f}% — 폭락 구간에서 전략이 실효성을 보였습니다. '
                               f'다만 Calmar {calmar:.2f}로 MDD 대비 수익 효율은 추가 점검이 필요합니다.')
                elif ep_rate is not None and calmar >= 0.5:
                    insight = (f'💰 Calmar {calmar:.2f}로 MDD 대비 수익은 양호합니다. '
                               f'에피소드 승률 {ep_rate:.0f}% — 일부 폭락 구간에서는 홀딩이 더 유리했습니다.')
                else:
                    ep_disp = f'{ep_rate:.0f}%' if ep_rate is not None else '집계 중'
                    insight = (f'⚠️ 백테스트 기간이 짧거나 현재 회복 중인 구간입니다. '
                               f'Calmar {calmar:.2f} / 에피소드 승률 {ep_disp} — '
...
**Q. Calmar Ratio는 뭔가요?**
`CAGR ÷ |포트폴리오 MDD|`예요. 예를 들어 CAGR 30%, MDD -20%이면 Calmar = 1.5예요. 숫자가 클수록 감수한 최대 낙폭 대비 연평균 수익이 효율적이라는 의미예요. 이 전략처럼 낙폭 구간에서 추가 매수하는 전략에는 Recovery Factor보다 시간을 반영한 Calmar가 더 적합해요.
